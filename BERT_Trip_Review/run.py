...
from model.pretrain import TripTrainer, TripReviewTrainer
...
if __name__ == "__main__":
    ...
    df_traj = pd.read_csv(f'./data/{args.dataset}/traj-{args.dataset}.csv')
    len_traj = len(df_traj)
    datasets = [(args.dataset, len_traj)]
    # datasets = [
    # #('osaka', 47 * 5),
    # #('melb', 442 * 2),
    # #('edin', 634 * 2),
    ...
    for (dataset, expected_size) in datasets:
        ...
        for fold in range(5):
            train_data, test_data, random_state  = get_kfold_data(df, fixed_random_state = fixed_random_state, fold=fold)
            ...
            for model in models:
                ...
                kw = kw | config
                m = TripReviewTrainer(base_model, **kw)
                # print('m', m)
